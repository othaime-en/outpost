...
**IAM implication:** the GitHub Actions role (`AWS_ROLE_ARN`) has
`ecs:UpdateService`, `ecs:DescribeServices`, `rds:StopDBInstance`,
`rds:StartDBInstance`, and `rds:DescribeDBInstances` attached, in addition
to what `terraform apply`/`destroy` already need - verified via real
pause/resume runs.
